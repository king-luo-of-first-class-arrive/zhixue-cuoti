...
分类规则：按题目考查的知识点分类，不要按运算符号分类。有生活情境、带单位、要写「答」的题属于「应用题」，只有光秃秃算式没有情境的才属于「计算」。
例如「长方形长5米宽3米求面积」属于「图形/算面积」，「鸡兔同笼」属于「应用题/鸡兔同笼」，「小明买5本书每本3元一共多少元」属于「应用题/乘除法应用题」，「1/2+1/3」属于「计算/分数计算」，「问1/2表示什么意思」才属于「数的认识」。
